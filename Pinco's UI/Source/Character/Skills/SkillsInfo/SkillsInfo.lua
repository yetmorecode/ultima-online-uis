...
function SkillsInfo.ClearGump()
	...
	if (DoesWindowExist(this .. "SkillDesc")) then
		DestroyWindow(this .. "SkillDesc")
	end
	if (DoesWindowExist(this .. "RequirementsLbLTitle")) then
		DestroyWindow(this .. "RequirementsLbLTitle")
	end
	if (DoesWindowExist(this .. "RequirementsLbLDescription")) then
		DestroyWindow(this .. "RequirementsLbLDescription")
	end
	if (DoesWindowExist(this .. "DirectLbLTitle")) then
		DestroyWindow(this .. "DirectLbLTitle")
	end
	if (DoesWindowExist(this .. "DirectLbLDescription")) then
		DestroyWindow(this .. "DirectLbLDescription")
	end
	if (DoesWindowExist(this .. "InterLbLTitle")) then
		DestroyWindow(this .. "InterLbLTitle")
	end
	if (DoesWindowExist(this .. "InterLbLDescription")) then
		DestroyWindow(this .. "InterLbLDescription")
	end
	if (DoesWindowExist(this .. "AutoLbLTitle")) then
		DestroyWindow(this .. "AutoLbLTitle")
	end
	if (DoesWindowExist(this .. "AutoLbLDescription")) then
		DestroyWindow(this .. "AutoLbLDescription")
	end
	if (DoesWindowExist(this .. "TtleLbLTitle")) then
		DestroyWindow(this .. "TtleLbLTitle")
	end
	if (DoesWindowExist(this .. "TtleLbLDescription")) then
		DestroyWindow(this .. "TtleLbLDescription")
	end
	if (DoesWindowExist(this .. "MoreLbLTitle")) then
		DestroyWindow(this .. "MoreLbLTitle")
	end
	if (DoesWindowExist(this .. "MoreLbLDescription")) then
		DestroyWindow(this .. "MoreLbLDescription")
	end
	if (DoesWindowExist(this .. "TrainLbLTitle")) then
		DestroyWindow(this .. "TrainLbLTitle")
	end
	if (DoesWindowExist(this .. "TrainLbLDescription")) then
		DestroyWindow(this .. "TrainLbLDescription")
	end
	if (DoesWindowExist(this .. "SatLbLTitle")) then
		DestroyWindow(this .. "SatLbLTitle")
	end
	if (DoesWindowExist(this .. "SatLbLDescription")) then
		DestroyWindow(this .. "SatLbLDescription")
	end
	if (DoesWindowExist(this .. "GolLbLTitle")) then
		DestroyWindow(this .. "GolLbLTitle")
	end
	if (DoesWindowExist(this .. "GolLbLDescription")) then
...
function lines(str)
	local t = {}
	local function helper(line) table.insert(t, line) return L"" end
	helper((str:gsub(L"(.-)\r?\n", helper)))
	return t
end
...
function SkillsInfo.UpdateGump(DisplaySkill )
	...
	local sat = LoadTextFile(path .. "satyr.txt")
	...
	if train then
		local tl = lines(train)

		train = L""

		local Satyr = false
		for i = 1, #tl do
			if tl[i] == L"|SATYR|" then
				Satyr = true
			else
				if tl[i] == "" then
					tl[i] = L""
		...
		if Satyr then
			...
			if sat then
				...
				LabelSetText( this .. "SatLbLTitle", GetStringFromTid(270)  )
