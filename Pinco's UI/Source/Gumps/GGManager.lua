...
function GGManager.destroyWindow( windowName, shouldUnregister )
	...
	if windowName ~= nil and DoesWindowExist(windowName) == true then	
		...
		if windowData ~= nil and windowData.gumpID ~= nil then
			...
			--UO_GenericGump.debug( towstring("called SaveWindowPosition, PositionGroup = "..savePositionGroup) )
		end
		
		DestroyWindow( windowName )
		--UO_GenericGump.debug( towstring("called DestroyWindow on windowName = "..windowName) )
	end
end
...
function GGManager.GGArrived()
	...
	if windowName and GGManager.onCreateCallback[windowName] then
		UO_GenericGump.debug( towstring("calling "..windowName.."'s onCreateCallback function ") )
		if GGManager.onCreateCallback[windowName]() == GGManager.NEW_GUMP_HAS_BEEN_HANDLED then	
			UO_GenericGump.debug( towstring(windowName)..L" has been handled by the callback. Exiting GGManager.GGArrived") 
			return;
		end
	end
	...
	--UO_GenericGump.debug( towstring("calling RestoreWindowPosition, PositionGroup = "..savePositionGroup) )

	-- *** could use this to access windows by index
	--WindowSetId( gumpName, GGManager.NumOfGumps )
	

	UO_GenericGump.debug( towstring("Created new window "..gumpName.." using template "..windowName) )
...
function GGManager.tableToString( data, spacing )
	...
	for k, v in pairs( data ) do
		...
		local keyName = GGManager.convertToWString( k ) or L"<"..towstring( type(k) )..L">"	
		local valueName
		
		if type( v ) == "table" then
			valueName = GGManager.tableToString( v, L"  "..spacing  )
		else
			valueName = GGManager.convertToWString( v ) or L"<"..towstring( type(v) )..L">"
		end
...
function GGManager.convertToWString( value )
	...
	elseif type( value ) == "string" then
		valueName = towstring( value )
	elseif type( value ) == "boolean" or type( value ) == "number" then
		valueName = towstring( tostring(value) )
	end
