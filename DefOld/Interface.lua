...
function Interface.Update( timePassed )
	...
	if totalTime > totalThres then
		avgPassed = totalTime / count
		perSecond = count / totalThres
		Debug.PrintToChat(L"Interface.Update() called " .. count .. L" times, " .. (math.floor(perSecond * 10) / 10) .. L" calls per second, average time between updates = " .. (math.floor(avgPassed * 1000) / 1000) .. L"s")
