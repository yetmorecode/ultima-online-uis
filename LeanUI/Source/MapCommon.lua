...
MapCommon.ZoomLevel[MapCommon.MAP_MODE_NAME].Max = 3.0
...
MapCommon.ContextReturnCodes.TILT_MAP = 3
MapCommon.ContextReturnCodes.CENTER_PLAYER = 4
MapCommon.ContextReturnCodes.SELECT_MAP = 5
...
function MapCommon.UpdateZoomValues(facet, area)
  if( MapCommon.ActiveView == MapCommon.MAP_MODE_NAME ) then
    local maxZoom = UORadarGetMaxZoomForMap(facet, area) * 2
...
function MapCommon.UpdateWaypointIcon(iconId,windowName,displayMode,waypointId, color)
  ...
  if(iconTexture ~= nil and iconWidth ~=nil and iconHeight ~=nil )then
    ...
    local lscale = 0.7
    WindowSetDimensions(windowName, iconWidth* iconScale * lscale, iconHeight* iconScale * lscale)
    DynamicImageSetTextureDimensions(windowName, iconWidth * lscale, iconHeight * lscale)
    DynamicImageSetTextureScale(windowName, lscale)
...
function MapCommon.DrawPlayer(displayMode, visibleFunc, parentWindow)
  ...
  if( playerVisible ) then
    ...
    local dx = playerX - 550
    dx = dx * 2
    playerX = 550 + dx
    local dy = playerY - 550
    dy = dy * 2
    playerY = 550 + dy
...
function MapCommon.CreateWPOnLocation(wname, wtype, wfacet, wx, wy, displayMode, waypointId, windowName, parentWindow)
  local waypointX, waypointY = UOGetWorldPosToRadar(wx, wy)
    ...
    local dx = waypointX - 550
    dx = dx * 2
    waypointX = 550 + dx
    local dy = waypointY - 550
    dy = dy * 2
    waypointY = 550 + dy
...
function MapCommon.ContextMenuCallback(returnCode, params)
  ...
  if( params ~= nil ) then
    ...
    elseif(returnCode == MapCommon.ContextReturnCodes.CENTER_PLAYER) then
      MapWindow.CenterOnPlayer = not MapWindow.CenterOnPlayer
      UORadarSetCenterOnPlayer(MapWindow.CenterOnPlayer)
    elseif(returnCode == MapCommon.ContextReturnCodes.TILT_MAP) then
      if not MapWindow.Tilt then
