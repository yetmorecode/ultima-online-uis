...
function Actions.ToggleMapWindow()
	...
	if(WindowData.HighlightEffect.windowToHighlight and string.find(WindowData.HighlightEffect.windowToHighlight, "Hotbar" .. Interface.MenuBar)) then
	...
	Interface.SaveBoolean( "MapVisible", Interface.MapVisible)
...
function Actions.ToggleLegacyContainers()
	...
	if(SystemData.Settings.Interface.LegacyContainers) then
   	...
   	if Interface.BackpackOpen then
...
function Actions.RequestContItems()
	...
	for i = 1, #AllItems  do
		...
		output = output .. towstring(i) .. L")\r\n"
		local props = ItemProperties.GetObjectProperties(AllItems[i], nil, "Actions - Request Container items list")
		if props then
			for i =1, #props do
				
				if (wstring.find(props[i], L"Weight")) then
					continue
				end

				if (wstring.find(props[i], L"Durability")) then
					continue
				end

				if (wstring.find(props[i], L"Item")) then
					continue
				end
				
				if (wstring.find(props[i], L"Insured")) then
					continue
				end

				if (wstring.find(props[i], L"Stygian")) then
					continue
				end
...
function Actions.ToggleLootbag()
	...
	--	WindowUtils.ChatPrint(GetStringFromTid(1155152), SystemData.ChatLogFilters.SYSTEM )
		--return	
	--end
	--Debug.Print(GetStringFromTid(1155153))
...
function Actions.LootbagTargetReceived()
	...
	-- BUG: Disable this until it can be fixed.	
	--if not IsContainer(objectId) then
	--	WindowUtils.ChatPrint(GetStringFromTid(1155159), SystemData.ChatLogFilters.SYSTEM )
		--return
	--end
