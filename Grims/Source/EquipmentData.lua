...
function EquipmentData.UpdateItemIcon(elementIcon, item)
    if( elementIcon ~= nil and item ~= nil and item.newWidth ~= nil and item.newHeight ~= nil ) then
		...
		if(item.customScale) then
			WindowSetDimensions(elementIcon, item.newWidth * item.customScale, item.newHeight * item.customScale)		
			DynamicImageSetTextureDimensions(elementIcon, item.newWidth *  item.customScale, item.newHeight * item.customScale)
			DynamicImageSetTextureScale(elementIcon, item.iconScale * item.customScale)				
		else
		...
		--local hueR,hueG,hueB,hueA = HueRGBAValue(hue)
