...
Interface.EnableCartographer = false
...
function Interface.LoadVariables()
	...
	Interface.EnableCartographer =						Interface.LoadBoolean( "EnableCartographer", Interface.EnableCartographer )
	Interface.MapVisible =								Interface.LoadBoolean( "MapVisible", Interface.MapVisible )
...
--	CreateWindow( "SkillsWindow2", false )
	CreateWindow( "LootConfiguration", false )
	CreateWindow( "VendorConfiguration", false )
...
function Interface.InitializeMap()
		...
		if Interface.MapVisible then	
			...
			if (MapCommon.ActiveView ~= nil) then
				...
				if (MapWindow.Big) then
					...
					WindowSetDimensions("MapWindow", Interface.LoadNumber( "MapWindowBigW" , 716), Interface.LoadNumber( "MapWindowBigH" , 776))
					MapWindow.OnResizeEnd("MapWindow")
				else
					MapZoom = Interface.LoadNumber( "MapZoom" , -100)
					local w = Interface.LoadNumber( "MapWindowW" , 400)
					local h = Interface.LoadNumber( "MapWindowH" , 400)
	...
	elseif Interface.NewPlayerMapToggled then
		if Interface.MapVisible then	
			SystemData.Settings.Interface.mapMode = MapCommon.MAP_ATLAS
			UserSettingsChanged()
			UOSetWaypointDisplayMode(MapCommon.RADAR_MODE_NAME)
...
function Interface.Update( timePassed )
	...
	if (Interface.DeltaTime >= 1) then
		...
		ok, err = pcall(Interface.BackpackItemsCheck, timePassed)
		Interface.ErrorTracker(ok, err)
...
function Interface.MapRefresh(timePassed)
	...
	if MapCommon.AreaDescription ~= "New Player Quest Area" then
		...
		if DoesWindowNameExist("RadarWindow") then
			DestroyWindow("RadarWindow")
		end
		if MapCommon.ActiveView ~= nil then
	...
	WindowSetShowing("MapWindow", Interface.MapVisible and (MapCommon.AreaDescription ~= "New Player Quest Area" or Interface.NewPlayerMapToggled))
end
...
function Interface.BackpackItemsCheck(timePassed)
	...
	if (WindowData.PlayerEquipmentSlot[EquipmentData.EQPOS_BACKPACK]) then
		...
		Interface.BackPackItems = ContainerWindow.ScanQuantities(playerBackpack, true)		
...
function Interface.Dump()
	Debug.Print("Taking a dump...")
	Debug.Print("==========================================")
...
function Interface.Test()
	...
	--GameData.UseRequests.UseItem = 1126503123
	--GameData.UseRequests.UseTarget = 0
	--BroadcastEvent(SystemData.Events.ITEM_USE_REQUEST)
	--WindowUtils.OnLButtonUp()
	--Interface.ItemUseRequest()
	--BroadcastEvent(SystemData.Events.UPDATE_PROCESSED)
	--BroadcastEvent(SystemData.Events.L_BUTTON_DOWN_PROCESSED)
	--HealthBarManager.OnEndDragHealthBar()
	--BroadcastEvent(SystemData.Events.WINDOWS_SETTINGS_UPDATED)
	--Interface.Update(1)
	--HotbarExecuteItem(2, 1)
end
...
function Interface.UpdateWindowsSettings()
	Debug.Print("Interface.UpdateWindowsSettings")
...
-- HERE
	Debug.Print(GameData.UseRequests)
	...
	if GameData.UseRequests.UseTarget ~= 0 then
		--Debug.Print(GameData.UseRequests.UseTarget)
		Interface.OnTarget(0, GameData.UseRequests.UseTarget)
	end
	Debug.Print("ObjectInfo ================")
	Debug.Print(WindowData.ObjectInfo.Type)
	...
	if itemData.containerId and itemData.containerId ~= 0 then
		Debug.Print("Trying to open container ".. itemData.containerId)
