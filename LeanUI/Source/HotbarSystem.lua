...
function HotbarSystem.Initialize()
	...
	for index, hotbarId in pairs(SystemData.Hotbar.HotbarIds) do
	  ...
	  if hotbarId < 1000 then
	    --Debug.PrintToChat(L"standard hotbar "..hotbarId)
		  HotbarSystem.SpawnNewHotbar(hotbarId)
		else
		  --Debug.PrintToChat(L"custom hotbar "..hotbarId)
		end
	end
...
function HotbarSystem.GetNextHotbarIdFrom(start)
...
function HotbarSystem.GetNextCustomHotbarId()
  return HotbarSystem.GetNextHotbarIdFrom(1000)
end

function HotbarSystem.GetNextHotbarId()
	return HotbarSystem.GetNextHotbarIdFrom(2)
end
...
function HotbarSystem.RegisterAction(element, hotbarId, itemIndex, subIndex)
	...
	if iconId ~= nil then
	 --Debug.PrintToChat(L"hotbar " .. hotbarId.. L" slot " .. itemIndex .. L" icon " .. iconId)
	else
	 --Debug.PrintToChat(L"hotbar " .. hotbarId.. L" slot " .. itemIndex .. L" icon nil")
	end
...
function HotbarSystem.UpdateActionButton(element, type, id, iconId, disabled )
	...
	elseif (type == SystemData.UserAction.TYPE_WEAPON_ABILITY) then
		...
		WindowSetAlpha(elementIcon, 0.7)
	end
...
function HotbarSystem.UpdateQuantity()
	...
	if( HotbarSystem.ObjectSlots[id] ~= nil ) then
		...
		for element, itemLoc in pairs(HotbarSystem.ObjectSlots[id]) do
			local elementIcon = element.."SquareIcon"
			if (item ~= nil and quantity > 1) then
				WindowSetShowing(element.."Disabled",false)
				ButtonSetDisabledFlag(element,false)
				
                EquipmentData.UpdateItemIcon(elementIcon, item)
				LabelSetText(element.."Quantity",L""..quantity)
			elseif (item ~= nil and quantity == 1) then
				WindowSetShowing(element.."Disabled",false)
				ButtonSetDisabledFlag(element,false)
...
function HotbarSystem.CreateUserActionContextMenuOptions(hotbarId, itemIndex, subIndex, slotWindow)
	...
	if( ( UserActionHasTargetType(hotbarId,itemIndex,subIndex) ) and ( SystemData.Settings.GameOptions.legacyTargeting == false ) ) then
		...
		ContextMenu.CreateLuaContextMenuItemWithString(L"Target: Self",0,HotbarSystem.ContextReturnCodes.TARGET_SELF,param,pressed[1+SystemData.Hotbar.TargetType.TARGETTYPE_SELF])
		ContextMenu.CreateLuaContextMenuItemWithString(L"Target: Cursor",0,HotbarSystem.ContextReturnCodes.TARGET_CURSOR,param,pressed[1+SystemData.Hotbar.TargetType.TARGETTYPE_CURSOR])
		ContextMenu.CreateLuaContextMenuItemWithString(L"Target: Stored",0,HotbarSystem.ContextReturnCodes.TARGET_OBJECT_ID,param,pressed[1+SystemData.Hotbar.TargetType.TARGETTYPE_OBJECT_ID])
		ContextMenu.CreateLuaContextMenuItemWithString(L"Target: Current",0,HotbarSystem.ContextReturnCodes.TARGET_CURRENT,param,pressed[1+SystemData.Hotbar.TargetType.TARGETTYPE_CURRENT])
...
function HotbarSystem.UpdateBinding(element,key)
	...
	if (wstring.find(key, L"Num Pad")) then
		key = wstring.gsub(key, L"Num Pad: ", L"#")
	end
...
function HotbarSystem.SetActionOnHotbar(actionType, actionId, actionIcon, hotbarNum, buttonNum)
	...
	--HotbarSystem.UpdateActionButton(element, actionType, actionId, actionIcon, true)
end