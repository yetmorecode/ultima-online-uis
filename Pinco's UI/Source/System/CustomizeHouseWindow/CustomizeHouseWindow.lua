...
function CustomizeHouseVisibility.SetNumOfStories( numOfStories )
	...
	--Debug.PrintToDebugConsole(L"    numOfStories = "..towstring( tostring( numOfStories ) ) )
...
function CustomizeHouseVisibility.SetCurrentStory( storyNum )
	...
	--Debug.PrintToDebugConsole(L"	storyNum = "..towstring( tostring(storyNum) ) )
	
	--Debug.PrintToDebugConsole( L" setting current ComboBoxSetSelectedMenuItem to storyNum = "..towstring( tostring(storyNum) )  )	
...
function CustomizeHouseWindow.Initialize()
	...
	LabelSetText( CustomizeHouseWindow.windowName.."RoofHeightText", towstring( tostring(CustomizeHouseWindow.roofZOffset) ) )
...
function CustomizeHouseWindow.getDoorSwingDirectionIcon( categoryID, iconIndex )
	...
	--Debug.PrintToDebugConsole(towstring("   returning icon="..CustomizeHouseWindow.DOOR_SWING_DIRECTION_ICONS[direction]..", position="..position..", yOffset="..yOffset) )
...
function CustomizeHouseWindow.displayRoofButtons( shouldDisplay )
	--Debug.PrintToDebugConsole(L"CustomizeHouseWindow.displayRoofButtons set to "..towstring( tostring( shouldDisplay ) ) )
...
function CustomizeHouseWindow.Update()
	...
	local cost = towstring( tostring( WindowData.CustomHouseInfo.Cost or 0 ) )
	...
	LabelSetText (CustomizeHouseWindow.windowName.."HouseCostText", L""..StringUtils.AddCommasToNumber (cost) )	
...
function CustomizeHouseWindow.OnIconSelected()
	...
	--Debug.PrintToDebugConsole( L"ustomizeHouseWindow.OnIconSelected ID = "..towstring( tostring(iconID) ) )
	...
	if (CustomizeHouseWindow.currentCategoryID ~= nil and CustomizeHouseWindow.entitlements[CustomizeHouseWindow.currentCategoryID] == false) then
		...
		local windowData = 
		{
			...
			closeCallback = okayButton.callback,
...
function CustomizeHouseWindow.displayIcons( category )
	...
	local iconIndex, iconID
	for dataIndex = 1, #category.icons do
		local iconData = category.icons[dataIndex]
		iconIndex = iconData[1]
		iconID = iconData[2]
...
function CustomizeHouseWindow.turnOffFrameAroundWindow( icon )
	...
	if icon ~= nil and DoesWindowExist( icon) then
...
function CustomizeHouseWindow.deleteIcons( windowNames )
	...
	for i = 1, #windowNames do
		local name = windowNames[i]
