...
function Hotbar.CreateNew(_, text)
	if tonumber(text) and not wstring.find(text, L"x") then
		...
		if wstring.find(text, L"x") then
			local slots = tostring(wstring.sub(text, 1, wstring.find(text, L"x") - 1))
			...
			if slots then
				...
				local bars = tostring(wstring.sub(text, wstring.find(text, L"x")+1, -1 ))
				bars = tonumber(bars)
				if bars then
					if bars > 12 then
						bars = 12
					...
					WindowUtils.ChatPrint( GetStringFromTid(1155385), SystemData.ChatLogFilters.SYSTEM)
