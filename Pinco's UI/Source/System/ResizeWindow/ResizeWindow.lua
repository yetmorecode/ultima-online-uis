...
function ResizeWindow.LockTooltip()
	if (ResizeWindow.Locked) then
		Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name, GetStringFromTid(1154933))
	else
		Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name, GetStringFromTid(1154932))
	end
end

function ResizeWindow.UpdateLockStatus()
	ResizeWindow.Locked = Interface.LoadSetting( "ResizeWindowLocked", ResizeWindow.Locked )
	local this = "ResizeWindow"
	
	local texture = "Lock"
	if not ResizeWindow.Locked  then
		texture = "UnLock"
	end
...
function ResizeWindow.Lock()
	...
	Interface.SaveSetting( "ResizeWindowLocked", ResizeWindow.Locked  )
	local this = "ResizeWindow"
	local texture = "Lock"
	if not ResizeWindow.Locked  then
		texture = "UnLock"
